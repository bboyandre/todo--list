...
const App = () => {
  ...
  const adicionarTextoDoInputNaLista = () => {
    ...
    setListaDeFrutas([...listaDeFrutas, {
      etiqueta: texto
    }])
    setTexto('')
  }
  ...
  const marcarFruta = (posicaoDaFruta) => {
    setListaDeFrutas(prev => [
      ...
      {
        ...prev[posicaoDaFruta],
        estaMarcado: !prev[posicaoDaFruta].estaMarcado
      },
  ...
  return ( <
    ...
    /> <
    button class = "buttonTexto pure-button"
    onClick = {
      adicionarTextoDoInputNaLista
    } > Salvar na Lista < /button> <
    /div> <
    ul class = " " > {
      ...
      listaDeFrutas.map((tarefa, posicaoDaTarefa) => (
        ...
        button class = "buttonDelete pure-button"
        onClick = {
          () => deletarItem(tarefa._id)
        } > Delete < /button>
